package sifive.fpgashells.devices.xilinx.xilinxmicrophasea7mig
...
import sifive.fpgashells.ip.xilinx.microphasea7mig.{MicrophaseA7MIGIOClocksReset, MicrophaseA7MIGIODDR, microphasea7mig}

case class XilinxMicrophaseA7MIGParams(
  address : Seq[AddressSet]
)

class XilinxMicrophaseA7MIGPads(depth: BigInt) extends MicrophaseA7MIGIODDR(depth) {
  def this(c : XilinxMicrophaseA7MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxMicrophaseA7MIGIO(depth: BigInt) extends MicrophaseA7MIGIODDR(depth) with MicrophaseA7MIGIOClocksReset

class XilinxMicrophaseA7MIGIsland(c: XilinxMicrophaseA7MIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  ...
  class Impl extends LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxMicrophaseA7MIGIO(depth)
    })
    ...
    val blackbox = Module(new microphasea7mig(depth))
...
class XilinxMicrophaseA7MIG(c: XilinxMicrophaseA7MIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxMicrophaseA7MIGIsland(c, crossing))
